...
- Hyperparameter Optimisation: Apply GWO to fine-tune the model’s parameters.
- Performance Comparison: Evaluate the optimised LSTM against baseline models (KNN, SVR).

## Key Technologies
- LSTM Network: A type of recurrent neural network suited for time series prediction.
- Grey Wolf Optimisation: A meta-heuristic algorithm inspired by the social hierarchy and hunting behavior of grey wolves.
...
## Results
The GWO-optimised LSTM model showed improved predictive accuracy over the unoptimised LSTM and baseline models in most evaluation metrics, particularly for Pfizer and Coca-Cola stocks. However, further optimisation could improve results, especially for more volatile stocks like Apple.
...
## Conclusion
This project demonstrates the potential of combining LSTM networks with GWO for stock price prediction, achieving significant improvements in accuracy over traditional methods. The results encourage further exploration into more sophisticated optimisation techniques and model architectures.
